...
from sam2.utils.limited_list import QueueList as qlist
...
class SAM2VideoPredictor(SAM2Base):
    ...
    def __init__(
        ...
        num_candidate_frame=3,
        obj_score_threshold=0.8,
        num_predictor_ref_frame=3,
        use_long_term_memory=False,
        num_long_mem_frame=3,
        num_long_mem_candidate=5,
        use_iou_to_find_long_mem=False,
        use_credible_initial_frame=True,
        num_cand_to_cond_frame=1,
        ...
        **kwargs,
        ...
        self.num_candidate_frame = num_candidate_frame
        self.num_cifs_candidate_frame = num_cifs_candidate_frame
        self.obj_score_threshold = obj_score_threshold
        self.num_predictor_ref_frame = num_predictor_ref_frame
        self.use_long_term_memory = use_long_term_memory
        self.num_long_mem_frame = num_long_mem_frame
        self.num_long_mem_candidate = num_long_mem_candidate
        self.use_iou_to_find_long_mem = use_iou_to_find_long_mem
        self.use_credible_initial_frame = use_credible_initial_frame
        self.num_cand_to_cond_frame = num_cand_to_cond_frame

        print(f"use_long_term_memory: {use_long_term_memory}, num_long_mem_frame: {num_long_mem_frame}, "
              f"num_long_mem_candidate: {num_long_mem_candidate}, use_iou_to_find_long_mem: {use_iou_to_find_long_mem}, "
              f"use_credible_initial_frame: {use_credible_initial_frame}, num_cand_to_cond_frame: {num_cand_to_cond_frame}")
    ...
    def _obj_id_to_idx(self, inference_state, obj_id):
        ...
        if allow_new_object:
            ...
            inference_state["output_dict_per_obj"][obj_idx] = {
                ...
                "long_mem": qlist(maxlen=self.num_long_mem_frame),
                "long_mem_candidate": qlist(maxlen=self.num_long_mem_candidate),
            }
    ...
    def add_new_text(
        ...
        """将expression进行emb提取，然后放到inference_state"""
        obj_idx = self._obj_id_to_idx(inference_state, obj_id)
        text_inputs_per_object = inference_state["text_inputs_per_obj"][obj_idx]  # one object has one text input
        ...
        # 调用clip，提取出text prompt embedding，直接仅计算一次
        # expression shape [batch_size, seq_len]
        ...
        # 这个函数在这里的作用是获取dummy output，is_cond的值不影响结果
    ...
    def propagate_in_video_preflight(self, inference_state):
        # 注意：只有调用了这个函数，才会将处理好的mask提取memory，放到init condition frame中
        # 把有mask prediction的per obj放到cond_frame_outputs中
        ...
        for obj_idx in range(batch_size):
            ...
            for is_cond in [False, True]:
                ...
                # referring cannot be determined by cond and non_cond, but needs to be determined by adding ref_frame
            ...
            # Here, it is required that there must be a cond frame, otherwise an error will be reported.
            # Check whether there is text_inputs_per_obj, if so, no error will be reported
            # check and make sure that every object has received input points or masks
            obj_output_dict = inference_state["output_dict_per_obj"][obj_idx]
            obj_text_inputs = inference_state["text_inputs_per_obj"].get(obj_idx, None)
            if len(obj_output_dict["cond_frame_outputs"]) == 0 and obj_text_inputs is None:
    ...
    def select_frame_to_output(self, inference_state, obj_output_dict, selected_frame_idx, selected_out):
        high_res_masks = torch.nn.functional.interpolate(
            selected_out["pred_masks"].to(inference_state["device"]),
            size=(self.image_size, self.image_size),
            mode="bilinear",
            align_corners=False,
        )
        maskmem_features, maskmem_pos_enc = self._run_memory_encoder(
            ...
            batch_size=1,  # run on the slice of a single object
            high_res_masks=high_res_masks,
            object_score_logits=selected_out["object_score_logits"],
            # these frames are what the user interacted with
            is_mask_from_pts=True,
        )
        selected_out["maskmem_features"] = maskmem_features
        selected_out["maskmem_pos_enc"] = maskmem_pos_enc
        obj_output_dict["cond_frame_outputs"][selected_frame_idx] = selected_out
    ...
    def propagate_in_video(
        ...
        # num_candidate_frame = 5
        num_candidate_frame = self.num_cifs_candidate_frame
        # Initialize all obj, each obj has a previous_ref_feats
        previous_ref_feats_per_obj = {obj_idx: qlist(maxlen=self.num_predictor_ref_frame-1) for obj_idx in range(batch_size)}
        previous_ref_pos_embeds_per_obj = {obj_idx: qlist(maxlen=self.num_predictor_ref_frame-1) for obj_idx in range(batch_size)}
        previous_ref_candidates_per_obj = {obj_idx: qlist(maxlen=num_candidate_frame) for obj_idx in range(batch_size)}
        ...
        for frame_idx in tqdm(processing_order, desc="propagate in video"):
            ...
            for obj_idx in range(batch_size):
                ...
                # A status check needs to be performed inside each obj to determine whether it is necessary to enter the conditioning frame.
                obj_output_dict = inference_state["output_dict_per_obj"][obj_idx]
                ...
                if len(obj_output_dict["cond_frame_outputs"]) == 0:
                    ...
                    current_out, pred_masks, current_vision_feats, current_vision_pos_embeds = (
                        self._run_single_frame_referring(
                            inference_state=inference_state,
                            ...
                            previous_ref_feats_list=previous_ref_feats_per_obj[obj_idx][:self.num_predictor_ref_frame-1],
                            previous_ref_pos_embeds_list=previous_ref_pos_embeds_per_obj[obj_idx][:self.num_predictor_ref_frame-1],
                        )
                    )
                    ...
                    if not self.use_credible_initial_frame:
                        ...
                        if current_out['object_score_logits'][0, 0].sigmoid() > 0.9:
                            ...
                            previous_ref_candidates_per_obj[obj_idx].pop()

                        if len(previous_ref_candidates_per_obj[obj_idx]) == num_candidate_frame:
                            cand = previous_ref_candidates_per_obj[obj_idx]
                            # Check that the candidate frames are continuous
                            is_continuous = True
                            for i in range(num_candidate_frame-1):
                                is_continuous = True if cand[i][0] - cand[i+1][0] == 1 else False
                                if not is_continuous:
                                    break
                            is_consistent = True

                            is_continuous = is_continuous and is_consistent
                            if is_continuous:
                                sorted_cand = sorted(cand, key=lambda x: x[1]['iou'][0], reverse=True)
                                # self.num_cand_to_cond_frame = 1
                                for selected_frame_idx, selected_out in sorted_cand[:self.num_cand_to_cond_frame]:
                                # selected_frame_idx, selected_out = cand[0]
                                    high_res_masks = torch.nn.functional.interpolate(
                                        selected_out["pred_masks"].to(inference_state["device"]),
                                        size=(self.image_size, self.image_size),
                                        mode="bilinear",
                ...
                # Once the conditioning frame is detected, SAM2 tracking begins.
                else:
                    if frame_idx in obj_output_dict["cond_frame_outputs"]:
                        ...
                        # The frame not in cond_frame_outputs is non_cond_frame_outputs
                        storage_key = "non_cond_frame_outputs"
                        if 'text_emb' in inference_state["text_inputs_per_obj"][obj_idx]:
                            text_emb_inputs = inference_state["text_inputs_per_obj"][obj_idx]['text_emb']
                        else:
    ...
    def _reset_tracking_results(self, inference_state):
        ...
        for v in inference_state["output_dict_per_obj"].values():
            ...
            v["ref_frame_outputs"].clear()
            v["long_mem"].clear()
            v["long_mem_candidate"].clear()
    ...
    def _get_image_feature(self, inference_state, frame_idx, batch_size):
        ...
        # 可以存储多个frame的feature，这里只存储了一个。"cached_features"不会被reset清空，init才会被清空，因此inference per obj可以复用
    ...
    # Rewrite a function for obtaining conditioned features during inference to add long-term memory
        ...
        if not is_init_cond_frame:
            if self.use_long_term_memory:
                ...
                if prev_output is not None:
                    # self.use_iou_to_find_long_mem = False
                    if self.use_iou_to_find_long_mem:
                        this_iou = torch.max(prev_output['ious'])
                        if this_iou > 0.95:
                            output_dict["long_mem_candidate"].append((prev_frame_idx, this_iou))
                    else:
                        output_dict["long_mem_candidate"].append((prev_frame_idx, 1))

                if len(output_dict["long_mem_candidate"]) == self.num_long_mem_candidate:
                    if len(output_dict["long_mem"]) > 0:
                        prev_long_mem_frame_idx = output_dict["long_mem"][-1]
                        prev_long_mem = output_dict["non_cond_frame_outputs"].get(prev_long_mem_frame_idx, None)
                    else:
                        prev_long_mem_frame_idx = sorted(output_dict["cond_frame_outputs"].keys())[-1]
                        prev_long_mem = output_dict["cond_frame_outputs"][prev_long_mem_frame_idx]
                    prev_long_mem_features = prev_long_mem["maskmem_features"].to(device, non_blocking=True)

                    cos_sim_list = []
                    for (prev_frame_idx, prev_frame_iou) in output_dict["long_mem_candidate"]:
                        frame_output = output_dict["non_cond_frame_outputs"].get(prev_frame_idx, None)
                        frame_features = frame_output["maskmem_features"].to(device, non_blocking=True)
                        cos_sim = torch.sum(torch.cosine_similarity(prev_long_mem_features, frame_features, dim=-1))
            ...
            # Step 1: Select the nearest condition frame
            ...
            # Step 2: Select the nearest non-condition frame
            # Add last (self.num_maskmem - 1) frames before current frame for non-conditioning memory
            # the earliest one has t_pos=1 and the latest one has t_pos=self.num_maskmem-1
            # We also allow taking the memory frame non-consecutively (with stride>1), in which case
            # we take (self.num_maskmem - 2) frames among every stride-th frames plus the last frame.
            # Step 2.1: Get the frame_idx of the nearest non-condition frame and its corresponding output,
            ...
            for t_pos in range(1, self.num_maskmem):
                ...
                short_mem.append(prev_frame_idx)

            # Step 2.2: According to the position t_pos in memory, get the corresponding maskmem_features and maskmem_pos_enc
            ...
            if self.use_long_term_memory:
                for long_mem_frame_idx in output_dict["long_mem"]:
                    if long_mem_frame_idx in short_mem:
                        continue
                    long_mem_output = output_dict["non_cond_frame_outputs"].get(long_mem_frame_idx, None)

                    long_mem_feats = long_mem_output["maskmem_features"].to(device, non_blocking=True)
                    to_cat_memory.append(long_mem_feats.flatten(2).permute(2, 0, 1))
                    long_mem_pos_enc = long_mem_output["maskmem_pos_enc"][-1].to(device)
                    long_mem_pos_enc = long_mem_pos_enc.flatten(2).permute(2, 0, 1)
                    long_mem_pos_enc = long_mem_pos_enc + self.maskmem_tpos_enc[6]  # 0是离当前帧最近的memory，重要性最高
                    to_cat_memory_pos_embed.append(long_mem_pos_enc)
    ...
    def _run_single_frame_referring(
        self,
        inference_state,
        output_dict,
        frame_idx,
        ...
        text_emb_inputs,
        prev_sam_mask_logits=None,
        previous_ref_feats_list: List = None,
        previous_ref_pos_embeds_list: List = None,
    ):
        ...
        assert text_emb_inputs is not None
        ...
        # 获取iou最大的obj_idx
        max_iou_obj_idx = ious.argmax()
        iou = ious[:, max_iou_obj_idx]
