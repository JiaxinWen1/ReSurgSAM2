...
from sam2.modeling.sam2_utils import get_1d_sine_pe, MLP, select_closest_cond_frames, MLPDropout
...
class SAM2Base(torch.nn.Module):
    def __init__(
        ...
        self.use_sp_bimamba = use_sp_bimamba
        self.use_dwconv = use_dwconv
        self.cross_modal_dropout = cross_modal_dropout
        self.cross_modal_hidden = cross_modal_hidden
        self.forward_text_emb = forward_text_emb
        self._build_sam_heads()
        self.max_cond_frames_in_attn = max_cond_frames_in_attn

        # 初始化language encoder和cross-modal fusion
        self.language_encoder_name = language_encoder_name
        if self.language_encoder_name == 'clip':
            self._build_cross_modal_modules()
        else:
    ...
    def _build_cross_modal_modules(self):
        ...
        if len(self.use_feature_level) == 1:
            print(f"Using mamba setting: use_mamba_attn={self.use_mamba_attn} use_bi_mamba={self.use_bimamba}, "
                  f"use_sp_bimamba={self.use_sp_bimamba}, use_dwconv={self.use_dwconv}")
            self.cross_modal_fusion = CrossModalFusionModule(
                self.hidden_dim,
                image_embedding_size=(image_embedding_size, image_embedding_size),
                use_feature_level=self.use_feature_level,
                depth=self.cross_attn_depth,
                bimamba=self.use_bimamba,
                use_sp_bimamba=self.use_sp_bimamba,
                use_dwconv=self.use_dwconv,
                dropout=self.cross_modal_dropout,
                use_mamba_attn=self.use_mamba_attn,
                num_temp_pos_embed=self.num_temp_pos_embed,
    ...
    def _build_sam_heads(self):
        ...
        self.sam_prompt_encoder = PromptEncoder(
            ...
            mask_in_chans=16,
            forward_text_emb=self.forward_text_emb,
        )
    ...
    def forward_image(self, img_batch: torch.Tensor):
        ...
        backbone_out["backbone_fpn_pre_conv"] = backbone_out["backbone_fpn"].copy()
    ...
    def _prepare_backbone_features(self, backbone_out, need_pre_conv=False):
        ...
        if need_pre_conv:
            pre_conv_feature_maps = backbone_out["backbone_fpn_pre_conv"][-self.num_feature_levels :]
            pre_conv_feature_maps = [x.flatten(2).permute(2, 0, 1) for x in pre_conv_feature_maps]
            return backbone_out, vision_feats, vision_pos_embeds, feat_sizes, pre_conv_feature_maps
    ...
    def _track_step(
        ...
        # These images have been repeated for each object in each frame.
