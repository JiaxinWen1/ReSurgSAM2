...
class Trainer:
    ...
    def run_train(self):
        ...
        while self.epoch < self.max_epochs:
            # pretraining the model, with freezing the backbone for the first few epochs
            model = getattr(self.model, 'module', self.model)

            if not hasattr(self, 'original_training_state'):
                self.original_training_state = {}
                for name, param in model.named_parameters():
                    self.original_training_state[name] = param.requires_grad
            sam2_name_list = [
                'image_encoder', 'mask_downsample', 'memory_attention', 'memory_encoder', 'sam_prompt_encoder',
                'sam_mask_decoder', 'obj_ptr_proj', 'obj_ptr_tpos_proj'
            ]
            if self.epoch < self.pretrained_epoch:
                print(f'epoch: {self.epoch}, freeze SAM2 parameters')
                for name, param in model.named_parameters():
                    for freeze_name in sam2_name_list:
                        freeze_name = '{}.'.format(freeze_name)
                        if freeze_name in name:
                            param.requires_grad = False
            else:
                if len(self.original_training_state) > 0:
                    for name, param in model.named_parameters():
...
def print_model_summary(model: torch.nn.Module, log_dir: str = ""):
    ...
    # # print the name fo non_trainable_parameters
    # for name, param in model.named_parameters():
